...
import { icp_bootcamp1_backend } from 'declarations/icp_bootcamp1_backend';
...
function App() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const c = await icp_bootcamp1_backend.getCandidates();
        const v = await icp_bootcamp1_backend.getVotes();
        setCandidates(c);
        setVotes(v);
      } catch (error) {
  ...
  async function checkVoterStatus() {
    ...
    try {
      const status = await icp_bootcamp1_backend.checkVoterStatus(voterName);
      setHasVoted(status);
  ...
  async function fetchResults() {
    try {
      const res = await icp_bootcamp1_backend.getResults();
      setResults(res);
      setShowResults(true);
    } catch (error) {
  ...
  async function vote(index) {
    ...
    setLoading(true);
    try {
      const result = await icp_bootcamp1_backend.vote(index, voterName);
      setMessage(result);
      ...
      const updatedVotes = await icp_bootcamp1_backend.getVotes();
